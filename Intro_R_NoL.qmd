...
-   R ([Version 4.2+](https://cran.r-project.org)) and RStudio ([Version 1.4+](https://posit.co/download/rstudio-desktop/))
...
First that all, we need to know **WHERE** are we working at. In other words, our working directory. To get information that information we just need to type **getwd()** in the script or the console.
...
::: callout-tip
When using the **setwd()** function, we need to use quotation marks to specify the correct address. For example: **setwd("Dropbox/Teaching/NOL_Itasca/Intro_R")**.
:::
...
nol_2023 <- c("st1", "st2", "st3", "st4", "st5")

for (i in 1:length(nol_2023)){ 
  
  print(paste0("Hi ", nol_2023[i], ", welcome to the Intro to R 2023!"))
